	...
	public static Set<String> keys(String key) {
		...
		try {
			...
			// if (jedis.exists(key)) {
				...
				value = null != value && !value.isEmpty() ? value : null;
...
//		//key = getKey(key);
	...
	public static long zrem(String key, double start, double end) {
		//key = getKey(key);
		long result = 0;
		Jedis jedis = null;
		try {
			...
			logger.debug("zrem {} = {},{}",key, start,end);
		} catch (Exception e) {
			logger.warn("zrem {} = {},{}", key, start,end, e);
		} finally {
			returnResource(jedis);
		}
		return result;
